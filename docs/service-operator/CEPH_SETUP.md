...
Hostnames of the storage nodes, taken from `servers.list` entries with `role: storage`. The monitor count must be *odd and ≥ 3* for quorum; in an ProductName deployment the same nodes serve as both Ceph monitors and OSDs.
